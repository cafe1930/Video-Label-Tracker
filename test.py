...
class BboxesContainer:
    ...
    def update_bbox(self, updating_bbox, updating_source):
        ...
        updating_manual_idx = updating_bbox.manual_idx
        ...
        if updating_source == 'auto':
            ...
            # ПОЧЕМУ??? признак рамки - то же самое имя класса и тот же самый автоматический индекс
            filter_condition = (self.bboxes_df['class_name']==updating_class_name)\
    ...
    def get_registered_bboxes(self):
        filter_condition = self.bboxes_df['manual_idx'] != -1
        return self.bboxes_df[filter_condition]
        #return [bbox for bbox in self.bboxes_df[filter_condition]['bbox']]
    ...
    def change_tracking_type(self, bbox, new_tracking_type):
        ...
        new_tracking_type - тип трекинга объекта ['auto', 'manual']
        '''
        updating_class_name = bbox.class_name
        updating_auto_idx = bbox.auto_idx
        updating_manual_idx = bbox.manual_idx
    ...
    def check_updated_bboxes(self):
        ...
        dissapeared_bboxes = self.bboxes_df[~filter_condition]
    ...
    def register_auto_bbox(self, class_name, auto_idx, object_description=''):
        '''
        Выполняется изменение manual_idx и добавление описания объекта
        '''
        filter_condition = (self.bboxes_df['class_name']==class_name)\
                & (self.bboxes_df['auto_idx']==auto_idx) #& (self.bboxes_df['object_description']==object_descr)
        ...
        try:
            self.tracking_classes_counter[class_name] += 1
        except:
            self.tracking_classes_counter[class_name] = 1
        
        manual_idx = self.tracking_classes_counter[class_name]
        index = filtered_df.index
        # почему-то иначе объъект не изменяется
        bbox = filtered_df['bbox'].iloc[0]
        bbox.manual_idx = manual_idx
        #self.bboxes_df['bbox'].iloc[0].manual_idx = manual_idx
        self.bboxes_df.loc[index, 'bbox'] = bbox
        
        self.bboxes_df.loc[index, 'manual_idx'] = manual_idx
...
if __name__ == '__main__':
    ...
    print()
    # register
    b_c.register_auto_bbox(class_name='person', auto_idx=1, object_description='ПИДОРАС')
    b_c.register_auto_bbox(class_name='person', auto_idx=2, object_description='ПИДОРАСИНА')
