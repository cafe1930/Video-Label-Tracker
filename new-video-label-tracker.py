...
class LabelNewBoxDialog(QDialog):
    ...
    def confirm_and_exit(self):
        ...
        Меняем имя класса и номер у рамки
            ...
            result = show_info_message_box(
                ...
                info_text=f'Рамка {self.current_bbox_name} для объекта {self.current_object_descr} уже есть на кадре. Перезаписать ее?',
        ...
        #print('DEBUG  LabelNewDialog.confirm_and_exit')
        #print(f'self.current_object_descr={self.current_object_descr}, self.current_class_name={self.current_class_name}, self.current_registered_idx={self.current_registered_idx}')
        #print('----------------------')
        self.close()
    ...
    def combobox_value_changed(self, value):
        ...
        #print('DEBUG  LabelNewDialog.combobox_value_changed')
        #print(f'value={value}')
        #print(f'self.current_object_descr={self.current_object_descr}, self.current_class_name={self.current_class_name}, self.current_registered_idx={self.current_registered_idx}')
        #print('----------------------')
...
class TestWindow(QMainWindow):
    def __init__(self, screen_width, screen_height, tracker_type='yolov8x.pt'):
        ...
        '''
        super().__init__()
        test_button = QPushButton('Зарегистрировать!')
        test_button.clicked.connect(self.register_new_object_handling)

    def register_new_object_handling(self):
        pass
...
'''
    ...
    def change_additional_tracker_handling(self):
        select_add_tracker_dialog = SelectModel(list(self.additional_trackers_create_functions_dict.keys()))
    ...
    def change_detector_handling(self):
        #models_names_list = ['yolov8n', 'yolov8s', 'yolov8m', 'yolov8l', 'yolov8x']
        select_detector_dialog = SelectModel(self.detectors_names_list)
    ...
    def update_bboxes_on_frame(self):
        ...
        self.corrected_bbox = self.frame_with_boxes.bbox_after_correction
